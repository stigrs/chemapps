    ...
    : geom(g), rot(r)
{
    ...
    if (pos != -1) {
        ...
        get_token_value(from, pos, "sigma_top", sigma, sigma);
        get_token_value(from, pos, "rmi_tor", rmi, rmi);
        get_token_value(from, pos, "pot_tor", pot, pot);
        get_token_value(from, pos, "freq_tor", freq, freq);
    }

    // Calculate reduced moment of inertia if needed:
    if ((rot_top.size() > 0) && rmi.empty()) {
        perform_analysis = true;
        rmi.resize(1);
        rmi(0) = red_moment_of_inertia();
    }
